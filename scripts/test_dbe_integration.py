...
def run_dbe_simulation():
    ...
    for i, row in market_data.iterrows():
        ...
        fm.update_market_data({symbol: row['close']})
        ...
        if not open_trade and i % 48 == 0:
            ...
            if open_trade:
                logger.info(f"Trade ouvert @ {row['close']:.2f} | SL: {params['stop_loss_pct']:.2%} | TP: {params['take_profit_pct']:.2%}")

        if i % (24 * 7) == 0: # Log hebdomadaire
            logger.info(dbe.get_status())

    # 3. Test de sauvegarde et de chargement
    logger.info("Test de la sauvegarde et du chargement de l'état...")
    state_path = os.path.join(TEST_SAVE_DIR, "dbe_state.pkl")
    dbe.save_state(state_path)

    dbe_loaded = DynamicBehaviorEngine(config=TEST_CONFIG['dbe'])
    dbe_loaded.load_state(state_path)

    # Vérifier que les états sont identiques
    assert dbe.current_step == dbe_loaded.current_step, "Le 'current_step' ne correspond pas!"
    assert dbe.risk_level == dbe_loaded.risk_level, "Le 'risk_level' ne correspond pas!"
    assert dbe.market_regime == dbe_loaded.market_regime, "Le 'market_regime' ne correspond pas!"
    assert len(dbe.trade_history) == len(dbe_loaded.trade_history), "L'historique des trades ne correspond pas!"
    logger.info("Sauvegarde et chargement de l'état validés.")
