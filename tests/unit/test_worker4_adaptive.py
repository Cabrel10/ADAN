...
def worker4_env():
    """Crée une instance de l'environnement pour le Worker 4 avec la configuration correcte."""
    main_config = load_config()
    workers_config_data = load_config("/home/morningstar/Documents/trading/bot/config/workers.yaml")
    worker_specific_config = workers_config_data['workers']['w4']

    # Surcharger le capital initial pour correspondre au tier du worker (ex: 750$ pour Tier 4)
    main_config['portfolio']['initial_balance'] = 750.0
    
    # Passer le worker_id et la configuration spécifique
    env = MultiAssetChunkedEnv(
        worker_id=3,  # ID pour le Worker 4
        config=main_config, 
...
def test_worker4_position_limit(worker4_env):
    """Vérifie que max_positions=4 est respecté via l'API publique."""
    env = worker4_env
    env.portfolio.reset()
    ...
    # 1. Ouvrir quatre positions (valides)
    ...
    # 2. Tenter d'ouvrir une cinquième position (doit être refusée)
    ...
    assert receipt5 is None, "La cinquième position aurait dû être refusée (limite de 4)"
    assert len(env.portfolio._get_open_positions()) == 4, "Le nombre de positions ouvertes ne doit pas changer"
