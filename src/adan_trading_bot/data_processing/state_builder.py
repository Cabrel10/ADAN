...
class StateBuilder:
    ...
    def build_observation(
        ...
        for tf in self.timeframes:
            ...
            # Ensure all required features are present in each asset's DataFrame
            for asset_df in asset_dfs:
                for feature in features:
                    if feature not in asset_df.columns:
                        logger.warning(
                            f"Feature '{feature}' not found in {tf} data, adding zeros"
                        )
                        asset_df[feature] = 0.0
            ...
            if current_idx < self.window_size:
                ...
                logger.warning(
                    f"Current index {current_idx} is less than window_size {self.window_size} for timeframe {tf}. Returning zero-padded observation for {tf}."
                )
                observations[tf] = np.zeros(
                    (self.window_size, len(features)), dtype=np.float32
                )
                continue
            ...
            for f in features:
                ...
                if upper_f in column_mapping:
                    ...
                    logger.debug(
                        f"Mapped feature: '{f}' -> '{column_mapping[upper_f]}'"
                    )
                else:
            ...
            try:
                window_data = df[mapped_features].iloc[
            ...
            if window_data.empty:
                logger.warning(
                    f"Window data is empty for timeframe {tf} at index {current_idx}. Returning zero-padded observation for {tf}."
                )
                observations[tf] = np.zeros(
                    (self.window_size, len(features)), dtype=np.float32
                )
                logger.debug(
                    f"Empty window_data for {tf}. Shape: {window_data.shape}, Dtypes: {window_data.dtypes.to_dict()}"
                )
                continue
            ...
            logger.debug(
                f"Observation for {tf} before normalization. Shape: {obs.shape}, Dtype: {obs.dtype}, Content: {obs.flatten()[:5]}..."
            )
            ...
            if self.normalize and tf in self.scalers and self.scalers[tf] is not None:
                ...
                logger.debug(
                    f"Observation for {tf} after normalization. Shape: {obs.shape}, Dtype: {obs.dtype}, Content: {obs.flatten()[:5]}..."
                )
        ...
        try:
            ...
            logger.debug(
                f"build_observation returning 3D array with shape: {stacked_obs.shape}"
            )
            return stacked_obs
        except Exception as e:
